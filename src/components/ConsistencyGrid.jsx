...
export default function ConsistencyGrid() {
  ...
  const { weeks, months, maxLevelSeen } = useMemo(() => {
    ...
    // Quartile thresholds over the window's non-zero days.
    ...
    const levelOf = (xp) => (xp <= 0 ? 0 : xp <= t1 ? 1 : xp <= t2 ? 2 : xp <= t3 ? 3 : 4)
  ...
  const showTip = (e, cell) => {
    const grid = e.currentTarget.closest('[data-heatmap]').getBoundingClientRect()
    const r = e.currentTarget.getBoundingClientRect()
    const label = `${parseKey(cell.key).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })} · ${cell.xp} XP`
    setTip({ x: r.left - grid.left + r.width / 2, y: r.top - grid.top, label })
  }
  ...
  return (
    <Card>
      ...
      <div className="overflow-x-auto">
        <div data-heatmap className="relative inline-block min-w-full">
          {tip && (
            <div className="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full whitespace-nowrap rounded border border-white/15 bg-[#111] px-2 py-1 text-[11px] text-slate-200"
              style={{ left: tip.x, top: tip.y - 6, fontFamily: MONO }}>
              {tip.label}
            </div>
          )}
          {/* Month labels */}
